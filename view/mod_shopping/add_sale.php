	...
	<div class="breadcrumbs">
		<div class="container">
			<ol class="breadcrumb breadcrumb1 animated wow slideInLeft" data-wow-delay=".5s">
				<li><a href="sales.php"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span>Atrás</a></li>
				<li><a href="../../index.php"><span class="glyphicon glyphicon-home" aria-hidden="true"></span>Inicio</a></li>
				<li class="active">Página de registrar nueva venta</li>
			</ol>
		</div>
	</div>
<!-- //breadcrumbs -->
			...
			<div class="login-form-grids">
				...
				<form method="post" enctype="multipart/form-data">
					<?php 
					 ...
					 ?>
					<input type="text" placeholder="Id factura venta" name="id_factura_venta" value="<? echo $rowfactura['maximo']+1?>" readonly >
					<input type="text" placeholder="Nombre" name="nombre" value="<? echo $_SESSION['nom_Usuario']?>" readonly >
					<?php 
						$correo = $_SESSION['cor_Usuario'];
						$sqlempleado = "SELECT id_empleado FROM empleado WHERE correo = '$correo'";
						$resulempleado = pg_query($conexion, $sqlempleado);
						$rowempleado = pg_fetch_array($resulempleado);
					 ?>
					 <input type="hidden" placeholder="Id empleado" name="id_empleado" value="<? echo $rowempleado['id_empleado']?>" readonly >
					...
					<select name="cedula" required >
						...
						<?php 
							...
							while ($row = pg_fetch_array($result)) {
								echo "
									<option value='$row[cedula]'>$row[nombre] $row[apellido]</option>
								";
							}
						 ?>
					</select>
					<input type="text" placeholder="Fecha venta" name="fecha_venta" value="<? echo date('Y-m-d') ?>" readonly >

					<input type="submit" name="btnregistred" value="Registrar venta">
				</form>
	...
	<?php 
		...
		if (isset($_POST['btnregistred'])) {
			...
			if ($result >= 1) {
				...
				echo "alert('Venta ingresada con éxito!');";
				echo "window.location.replace('sales.php');";
				echo "</script>";	
			}
			else{
				echo "<script type='text/javascript'>";
				echo "alert('No se pudo insertar datos de la venta...');";
				echo "</script>";		
			}
		}
		//Cerrar la conexón
